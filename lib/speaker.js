...
Speaker.prototype.write = function (chunk) {
  ...
  if (!this._audioStream.write.apply(this._audioStream, arguments))
    this.cork();
};
...
Speaker.prototype._openSpeaker = function _openSpeaker() {
  ...
  this._audioStream = sox.stdin
    ...
    .once('end', () => {
      debug('sox stream end');
      this._audioStream = null;
    })
    .once('close', () => {
      debug('sox stream close');
      this.emit('close');
    })
    .once('finish', () => {
      debug('sox stream finish');
      this.emit('finish');
    })
    .on('drain', () => {
      debug('sox stream drain');
      this.emit('drain');
    });
};
